/****** Object:  StoredProcedure [cons_retail].[sp_store_kpi_weekly]    Script Date: 12/20/2022 1:51:43 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROC [cons_retail].[sp_store_kpi_weekly] @jobid [int],@step_number [int],@reset [bit],@pipelineid [varchar](500) AS BEGIN BEGIN TRY IF @reset = 0 BEGIN 
...
IF OBJECT_ID('tempdb..#storekpiweekly') IS NOT  NULL
BEGIN
    DROP TABLE #storekpiweekly
END
create table #storekpiweekly
with
...
/*****aggregating store_kpi data at week level*****/
SELECT
  CAST(
    concat(
      format(
...
group by
  ...
  ) OPTION (LABEL = 'AADPRETKPISTOKPIWKLY');



MERGE
INTO cons_retail.store_kpi_weekly as TargetTbl
USING #storekpiweekly as SourceTbl
...
SET
  @label = 'AADPRETKPISTOKPIWKLY' EXEC meta_ctl.sp_row_count @jobid,
  @step_number,
  @label
END
...
SELECT
  ...
  'cons_customer.sp_store_kpi_weekly' AS ErrorProcedure,
