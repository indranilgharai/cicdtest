/****** Object:  StoredProcedure [cons_retail].[sp_store_consultant_kpi_weekly]    Script Date: 12/20/2022 1:49:43 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROC [cons_retail].[sp_store_consultant_kpi_weekly] @jobid [int],@step_number [int],@reset [bit],@pipelineid [varchar](500) AS BEGIN BEGIN TRY IF @reset = 0 BEGIN 
...
from
	cons_retail.store_consultant_kpi_weekly;

IF OBJECT_ID('tempdb..#storeconsultantweekly') IS NOT  NULL
BEGIN
    DROP TABLE #storeconsultantweekly
END
create table #storeconsultantweekly
with
...
/*****aggregating store_consultant_kpi data at week level*****/
SELECT
    CAST(
        concat(
            format(
...
FROM
    [cons_retail].[store_consultant_kpi_daily]c
	where date_key>=   cast(
        DATEADD(
            d,
...
GROUP BY
    ...
    OPTION (LABEL = 'AADPRETKPICONSWKLY');

	
MERGE
INTO cons_retail.store_consultant_kpi_weekly as TargetTbl
USING #storeconsultantweekly as SourceTbl
...
SET
    @label = 'AADPRETKPICONSWKLY' EXEC meta_ctl.sp_row_count @jobid,
    @step_number,
    @label
END
...
SELECT
    ...
    'cons_customer.sp_store_consultant_kpi_weekly' AS ErrorProcedure,
