...
CREATE PROC [std].[sp_netsuite_subsidiary_combined] @jobid [int],@step_number [int],@reset [bit],@pipelineid [varchar](500) AS 
BEGIN
	BEGIN TRY
		IF @reset=0
		BEGIN
			...
			With subsidiary as (
			select distinct  *,rank() OVER (PARTITION BY country order by lastmodifieddate desc,md_record_ingestion_timestamp desc) as dupcnt
			from [stage].[netsuite_subsidiary]
			)
			INSERT INTO [std].[netsuite_subsidiary_combined]
			select distinct 
			 FORMAT(CAST(sbs_no AS INT),'00','en-US') as sbs_no
			,sbs_code as sbs_code
			,sbs_code_short as sbs_code_short
			,sbs_dp_code_short as sbs_dp_code_short
			,sbs_olympic_code as sbs_olympic_code
			,sbs_name as sbs_name
			,sbs_region as sbs_region
			,sbs_report_region as sbs_report_region
			,sbs_currency_code as sbs_currency_code
			,sbs_currency_name as sbs_currency_name
			,sbs_currency_symbol as sbs_currency_symbol
			,sbs_currency_decimal as sbs_currency_decimal
			,sbs_currency_separator as sbs_currency_decimal
			,gmt_offset as gmt_offset
			,sbs_order as sbs_order
			,sbs_fy_start as sbs_fy_start
			,sbs_active as sbs_active
			,store_budgets_flag as store_budgets_flag
			,support_email as support_email
			,hybris_site_id as hybris_site_id
			,sbs_warehouse as sbs_warehouse
			,sbs_warehouse_code as sbs_warehouse_code
			,country as country
			...
			from std.subsidiary_x sbsx 
			full join (select * from subsidiary where dupcnt=1) sb on sbsx.sbs_dp_code_short=sb.country ;

			IF OBJECT_ID('tempdb..#netsuite_subsidiary_combined_temp') IS NOT NULL
			BEGIN
				DROP TABLE #netsuite_subsidiary_combined_temp
			END
			
			-----------------------temporary table to deduplicate data ---------------------------
			
			create table #netsuite_subsidiary_combined_temp WITH( DISTRIBUTION = ROUND_ROBIN,HEAP) AS
			select [sbs_no],
				[sbs_code],
				[sbs_code_short],
				[sbs_dp_code_short],
			...
			from (SELECT *, rank() OVER (PARTITION BY id ORDER BY [lastmodifieddate] desc,md_record_written_timestamp desc) AS dupcnt
				FROM std.netsuite_subsidiary_combined )a WHERE dupcnt=1 ;

			truncate table std.netsuite_subsidiary_combined;
			
			insert into std.netsuite_subsidiary_combined
			select * from #netsuite_subsidiary_combined_temp			
			OPTION (LABEL = 'AADSTDSBSCOM');

			DROP TABLE  #netsuite_subsidiary_combined_temp;
				--BELOW SCRIPT TO GET THE DRIVER TABLE READ COUNT AND TARGET TABLE WRITE COUNT
			DECLARE @label varchar(500)
			SET @label='AADSTDSBSCOM'
		...
		BEGIN
			...
			select @newrec=max(md_record_written_timestamp) from [std].netsuite_subsidiary_combined;
			
			delete from std.netsuite_subsidiary_combined where md_record_written_timestamp=@newrec;
...
BEGIN CATCH
	...
	'std.sp_netsuite_subsidiary_combined' AS ErrorProcedure ,
